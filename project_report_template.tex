...
\vspace{0.5cm}
\noindent\textbf{Keywords:} data science, Python, machine learning, Bitcoin, sentiment analysis, financial prediction, time series
...
\begin{lstlisting}[caption={Construction of lagged features}]
...
As an exploratory analysis, the correlation between lagged news sentiment and daily Bitcoin returns was also examined. The observed correlation (0.025) is weak and close to zero, indicating the absence of a strong linear relationship. This result is consistent with the visual dispersion observed in the sentiment–return scatter plot and further highlights the noisy nature of the relationship between news sentiment and short-term price movements.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{results/sentiment_distribution.png}
\caption{Distribution of daily aggregated news sentiment}
\label{fig:sentiment_dist}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{results/sentiment_vs_return.png}
\caption{Relationship between lagged sentiment and daily Bitcoin returns}
\label{fig:sentiment_return}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{results/rolling_sentiment_return.png}
\caption{Seven-day rolling averages of sentiment and Bitcoin returns}
\label{fig:rolling}
\end{figure}
...
\begin{itemize}
    \item \texttt{data\_loader.py}: data loading and preprocessing
    \item \texttt{models.py}: model definitions and training
    \item \texttt{evaluation.py}: performance metrics and evaluation
    \item \texttt{main.py}: execution pipeline and result generation
\end{itemize}

All experiments can be reproduced by running the \texttt{main.py} script after installing the required dependencies.
